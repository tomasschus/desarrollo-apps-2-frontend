...
describe("WhatToDoModal", () => {
  ...
  it("renders WhatToDoModal when open", () => {
    ...
    expect(screen.getByText("Describe la actividad que tienes en mente")).toBeInTheDocument();
  });
  ...
  it("does not render when closed", () => {
    ...
    expect(screen.queryByText("¿Qué te gustaría hacer?")).not.toBeInTheDocument();
  });
  ...
  it("shows error for invalid activity", async () => {
    ...
    const activityInput = screen.getByPlaceholderText("Ej: Visitar un museo, ir al teatro...");
    ...
    expect(screen.getByText("La descripción debe tener al menos 5 caracteres")).toBeInTheDocument();
  });
});
